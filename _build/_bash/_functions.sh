...
# ----------------------------------
# FUNC: lấy đường dẫn hiện tại của file build
# ----------------------------------
function get_source_code_path() {
  CURRENT_PATH=$(pwd)

  # Kiểm tra môi trường
  case "$OSTYPE" in
      linux*) 
          # Đường dẫn hiện tại không cần thay đổi trên Linux
          ;;
      msys* | cygwin* | mingw* ) 
          # Đường dẫn hiện tại cần được thay đổi trên Windows
          SOURCE_CODE_PATH=$(echo "$CURRENT_PATH" | sed 's/\/c/C:/') ;;
      *)
...
# ----------------------------------
# FUNC: Get lpp_lift_one_api_web source_code path
# ----------------------------------
function get_relative_path() {
  ...
  case "$OSTYPE" in
      ...
      msys* | cygwin* | mingw* ) 
          # Đường dẫn hiện tại cần được thay đổi trên Windows
          SOURCE_CODE_PATH=$(echo "$PARENT_PATH" | sed 's/\/c/C:/') ;;
      *)
          echo "Hệ điều hành không được hỗ trợ."
          exit 1
          ;;
